...
| Alpine Linux rootfs base | Mixed (per-package; no single blanket license) | Docker Hub `library/alpine`; exact tag + arm64 digest in `build/sources.lock` | `build/init-rootfs.sh` verifies the approved manifest; scratch-build patches embed the immutable digest-only pull reference in the runtime, and packaging rejects floating references. Only the trimmed package set in `build/alpine-packages.trimmed.txt` is included — see `build/AUDIT.md` for what was cut and why. Each Alpine package carries its own upstream license; consult the [Alpine package license index](https://pkgs.alpinelinux.org/packages) for a given package's specific license if needed.
