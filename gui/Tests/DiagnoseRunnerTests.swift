...
@Test func expandedReportProducesPrivacySafeDeveloperRows() throws {
    ...
    #expect(report.ntfsmacVersion == "1.0")
    #expect(report.buildVersion == "1")
    #expect(report.macosVersion == "26.6")
    #expect(report.architecture == "arm64")
    #expect(report.helperInstalled == true)
    #expect(report.vpnDefaultRoute == true)
    #expect(report.nfsMountCount == 1)
    #expect(rows.map(\.id) == ["version", "system", "binaries", "quarantine", "kernel", "bridge", "helper", "vpn", "mounts"])
...
@Test func expandedSystemRowWarnsForUnsupportedMacOS() {
