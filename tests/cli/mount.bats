...
@test "GUI mode returns a credential-required result for whole-disk BitLocker drive (disk4 on row 0)" {
  cat > "$STUB_DIR/anylinuxfs" <<STUB
#!/bin/bash
if [[ "\$1" == "list" ]]; then
  echo '   0:                  BitLocker                        *16.1 GB    disk4'
  exit 0
fi
echo "\$@" >> "$CALL_LOG"
exit 0
...
@test "BitLocker recovery key works with whole-disk device (disk4)" {
  ...
  [ "$status" -eq 0 ]
  run cat "$CALL_LOG"
  [[ "$output" == *"mount /dev/disk4"* ]]
  [[ "$output" == *"--key-file"* ]]
}

@test "interactive CLI prompt captures BitLocker credential when stdin is a terminal" {
  cat > "$STUB_DIR/anylinuxfs" <<STUB
#!/bin/bash
if [[ "\$1" == "list" ]]; then
  echo '   1:                  BitLocker Encrypted                3.0 TB     disk4s1'
...
proc = subprocess.Popen(['$SCRIPT', 'disk4s1'], stdin=slave, stdout=slave, stderr=slave)
os.close(slave)
buf = b''
while b'Enter BitLocker' not in buf:
    buf += os.read(master, 1024)
os.write(master, b'test-interactive-key\n')
proc.wait(timeout=5)
os.close(master)
"
  run cat "$CALL_LOG"
  [[ "$output" == *"--key-file"* ]]
  [[ "$output" != *"test-interactive-key"* ]]
}
